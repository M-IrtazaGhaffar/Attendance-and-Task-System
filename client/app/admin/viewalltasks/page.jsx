...
export default function GetTasks() {
    ...
    const fetchTasks = async () => {
        try {
            const res = await adminApi.post("/getalltasks");

            if (res.data && res.data.data) {
                toast.success('Fetched Your Tasks!')
        ...
        <div className="flex flex-col gap-4 w-full">
            ...
            <ScrollArea className="h-[750px] w-full p-4 border rounded-2xl shadow-2xl">
                <div className="flex flex-col gap-6 p-4 w-full">
                    {events.map((event, i) => (
                        <Link href={`/admin/viewalltasks/${event.id}`} key={event.uuid || event.id || i} className="flex gap-4 w-full">
                            {/* Timeline marker */}
